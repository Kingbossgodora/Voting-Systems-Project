...
sheet_id = '1EyV_fTr9Z-SSYt3oJqAeG_T5g5IAxof5WkG40wjrR5Y'
sheet_name = 'VotingRanked'
...
df.iloc[:, 1:33] = df.iloc[:, 1:33].applymap(lambda x: 5 - x)
...
for i, obj in enumerate(objects):
    ...
    obj.to_json(f'./json_files_scoring/object_{i}.json', orient='records')

# Iterate over each JSON file
for i in range(8):
    input_file_name = f'./json_files_scoring/object_{i}.json'
    output_file_name = f'./json_files_scoring/object_{i}.json'
    ...
    with open(input_file_name, 'r') as input_file:
        ...
        for obj in data:
            ...
            reformatted_data[order] = {
                'coordinates': [],
                'vote': [[choice, score] for choice, score in sorted_choices]
            }
